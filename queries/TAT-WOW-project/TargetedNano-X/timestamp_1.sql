...
SELECT  relevant_samples.ewh_sample_id
        ...
        ,GROUP_CONCAT(DISTINCT(sub.friendly_name) SEPARATOR ';') AS study_friendly_names
        ,GROUP_CONCAT(DISTINCT(studies.created) SEPARATOR ';') AS studies_set_up
FROM
(
  -- All samples that have had a Targeted Nano submission, and have been sequenced using a relevant primer panel
